%Alla Chepurova
% query to run - time(run(Steps, Moves, 100)).
...
touchdown_in_vis_area(Y, X, ProbY, ProbX, Move):-
    ...
    B is abs(ProbY - Y), A < 2, B < 2 -> step(Y, X, ProbY, ProbX, Move, Dir).
...
%==========recursive base - is touchdown has distance of two cell from us==========
rugby(Y, X, [Move]):-
